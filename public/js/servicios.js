$(function(){
    ...
    $("#searchreg").on("click", function (e) {
        ...
        $.post(RUTA + 'servicios/getAllItems',function(data,textStatus,jqXHR){
    ...
    //termina unidad de medida
     ...
     $("#seekGroups").on('click', function (e) {
        ...
        $.post(RUTA + 'servicios/listGroups',
    ...
    $("#dataItem").on("submit", function (e) {
        ...
        if (accion == 'n' ) {
            $.post(RUTA+"servicios/newItem", data,
                function (data, textStatus, jqXHR) {
                    if (data){
            ...
            $.post(RUTA+"servicios/updateItem", data,
                function (data, textStatus, jqXHR) {
                    if (data){
                        //grabar los datos de documentos
                        registerDosc(data);
    ...
    $("#addDocument").on("submit", function (e) {
        ...
        $.ajax({
            ...
            url: RUTA + 'servicios/getDataFile',
            // Request type
            type: "POST", 
     ...
     $("#tableSeekItems").on("click",'a', function (e) {
        ...
        $.post(RUTA + "servicios/searchById", {codigo:$(this).attr("href")},
            function (data, textStatus, jqXHR) {
                //pestaña 1
      ...
      $("#btnConfirm").on("click", function (e) {
        ...
        $.post(RUTA+"servicios/deleteItem", {codigo:$("#index_producto").val()},
            function (data, textStatus, jqXHR) {
        ...
        switch (activarForm) {
            ...
            case 5:
                ...
                if ( $("#index_producto").val().length > 0 ) {
                    $.post(RUTA + "servicios/listDocs", { data:$("#index_producto").val() },
                        function (data, textStatus, jqXHR) {
...
function desactivarCampos(){
    ...
    $("form")[1].reset();
    $(this).addClass('desactivado');
    $(".floatingButton, .formbody, #savereg, #cancelreg, #editreg, #deletereg").addClass('desactivado');
    $("#newreg, #searchreg, #exportexcel, #home").removeClass('desactivado');
    $(".seleccion").fadeOut();
    $(".formulario table tbody").empty();
}